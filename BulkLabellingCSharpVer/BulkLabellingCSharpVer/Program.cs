...
class MainClass
{
	...
	public static string directoryName;			//the directory name
	public static string trainingFileName;		//the image name
	public static string labelTag;				//the label tag of this item
	public static string finalFileName = "0";		//directory + image names
	...
	public static string FinalizeDirectoryName(int entryNum, string directoryName, string labelTag, string trainingFileName)
	{
		...
		string directoryAndImageName;	//directory + image names, to be used for return function
		char YesOrNoFinalFileName;		//directory + image + label name check

		//combine both directory and training file with label tag. it is to be used as a confirmation component only
		finalFileName = directoryName + trainingFileName + " " + labelTag;

		//combine the directory and training file names
		directoryAndImageName = directoryName + trainingFileName;
	...
	public static void WriteDirectoryData (string fileName)
	{
		...
		for (int i = 1; i < entryNum + 1; i++) 
		{
			...
			writer.WriteLine (finalFileName + i + " " + labelTag);
		}
		//close the text file
		writer.Close ();
